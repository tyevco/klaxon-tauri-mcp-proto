...
impl AlertStore {
  ...
  pub async fn create(&self, kind: String, threshold: f64, level: String, message: String) -> AlertRule {
    ...
    let rule = AlertRule { id, kind, threshold, level, message, enabled: true, last_fired_at: None };
    let _ = self.events.send(AlertEvent::Updated);
    rule
  }

  pub async fn update(&self, id: i64, kind: String, threshold: f64, level: String, message: String, enabled: bool) {
    let _ = sqlx::query(
  ...
  pub async fn delete(&self, id: i64) {
    let _ = sqlx::query("DELETE FROM alert_rules WHERE id = ?").bind(id).execute(&self.pool).await;
    let _ = self.events.send(AlertEvent::Updated);
  }
