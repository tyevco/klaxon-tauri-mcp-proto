...
use crate::{
  alert_store::{AlertStore, AlertEvent},
  checkpoint_store::{CheckpointStore, CheckpointEvent},
  logtail_store::{LogTailStore, LogTailEvent},
  mcp_http::{start_mcp_server, AgentInfo},
  models::{KlaxonLevel, KlaxonStatus},
  queue_store::{QueueStore, QueueEvent},
  scratchpad_store::{ScratchpadStore, ScratchpadEvent},
  ...
  toollog_store::{ToolLogStore, ToolLogEvent},
};
...
#[tauri::command]
async fn alerts_list(state: tauri::State<'_, Arc<AlertStore>>) -> Result<Vec<crate::alert_store::AlertRule>, String> {
  Ok(state.list().await)
}
...
#[tauri::command]
async fn queue_list(state: tauri::State<'_, Arc<QueueStore>>) -> Result<Vec<crate::queue_store::WorkItem>, String> {
  Ok(state.list().await)
}
...
async fn session_summary(
  ...
  let last_decision = store.list_answered(1).await
    .into_iter()
    .next()
...
async fn demo_seed_inner(
  ...
  tokens.add(TokenDelta { model: "claude-opus-4-6".into(),   input_tokens: 12_840,  output_tokens: 2_460,  cost_usd: Some(1.89), source: Some("code-review".into()) }).await;
  tokens.add(TokenDelta { model: "claude-sonnet-4-6".into(), input_tokens: 52_300,  output_tokens: 11_200, cost_usd: Some(0.55), source: Some("pr-142".into()) }).await;
  tokens.add(TokenDelta { model: "claude-haiku-4-5".into(),  input_tokens: 145_000, output_tokens: 31_000, cost_usd: Some(0.10), source: Some("pr-142".into()) }).await;
  tokens.add(TokenDelta { model: "claude-sonnet-4-6".into(), input_tokens: 8_000,   output_tokens: 1_500,  cost_usd: Some(0.08), source: Some("lint-fix".into()) }).await;

  // Demo scratchpad
  scratchpad.add("Agent started working on PR review backlog (47 PRs)".into(), "agent".into()).await;
  scratchpad.add("Skip PRs from bots — I'll filter those out".into(), "user".into()).await;
  scratchpad.add("Noted. Filtering bot-authored PRs. Starting with highest-risk changes.".into(), "agent".into()).await;

  // Demo checkpoints
  checkpoints.create("Fetched PR list".into(), Some("Retrieved 47 open PRs from GitHub".into()), Some(10), Some("pr-review".into())).await;
  checkpoints.create("Filtered bot PRs".into(), Some("12 bot PRs excluded, 35 remain".into()), Some(25), Some("pr-review".into())).await;
  checkpoints.create("Reviewed PROJ-142".into(), Some("High risk: auth changes detected".into()), Some(40), Some("pr-review".into())).await;
  checkpoints.create("Reviewing PROJ-156".into(), Some("In progress...".into()), Some(45), Some("pr-review".into())).await;

  // Demo log tail
  logtail.append(vec![
    "$ tsc --noEmit".into(),
    "src/api.ts:42:10 - error TS2322: Type 'string' is not assignable to type 'number'".into(),
    "Found 1 error.".into(),
  ], "stderr".into()).await;
  logtail.append(vec![
    "Running eslint...".into(),
  ...
  // Demo queue
  queue.push("Review PR #142 auth changes".into(), Some("Session TTL modification — needs security review".into()), 10, Some("claude-code".into())).await;
  queue.push("Review PR #156 API refactor".into(), Some("Large refactor, ~400 lines changed".into()), 8, Some("claude-code".into())).await;
  queue.push("Update changelog".into(), None, 5, Some("claude-code".into())).await;
  queue.push("Run full test suite".into(), Some("After all PR reviews complete".into()), 3, Some("claude-code".into())).await;
}
...
fn main() {
  tauri::Builder::default()
    .setup(|app| {
      ...
      let scratchpad_store = Arc::new(tauri::async_runtime::block_on(ScratchpadStore::new((*pool).clone())));
      app.manage(scratchpad_store.clone());

      let checkpoint_store = Arc::new(tauri::async_runtime::block_on(CheckpointStore::new((*pool).clone())));
      ...
      let alert_store = Arc::new(tauri::async_runtime::block_on(AlertStore::new((*pool).clone())));
      app.manage(alert_store.clone());

      let queue_store = Arc::new(tauri::async_runtime::block_on(QueueStore::new((*pool).clone())));
      ...
      tauri::async_runtime::spawn(async move {
        loop {
          match rx.recv().await {
            Ok(ScratchpadEvent::Updated) => { let _ = app_handle.emit("scratchpad.updated", serde_json::Value::Null); }
            Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => continue,
            Err(_) => break,
          }
        }
      ...
      tauri::async_runtime::spawn(async move {
        loop {
          match rx.recv().await {
            Ok(CheckpointEvent::Updated) => { let _ = app_handle.emit("checkpoints.updated", serde_json::Value::Null); }
            Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => continue,
            Err(_) => break,
          }
        }
      ...
      tauri::async_runtime::spawn(async move {
        loop {
          match rx.recv().await {
            Ok(LogTailEvent::Updated) => { let _ = app_handle.emit("logtail.updated", serde_json::Value::Null); }
            Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => continue,
            Err(_) => break,
          }
        }
      ...
      tauri::async_runtime::spawn(async move {
        loop {
          match rx.recv().await {
            Ok(ToolLogEvent::Updated) => { let _ = app_handle.emit("toollog.updated", serde_json::Value::Null); }
            Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => continue,
            Err(_) => break,
          }
        }
      ...
      tauri::async_runtime::spawn(async move {
        loop {
          match rx.recv().await {
            Ok(AlertEvent::Updated) => { let _ = app_handle.emit("alerts.updated", serde_json::Value::Null); }
            Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => continue,
            Err(_) => break,
          }
        }
      ...
      tauri::async_runtime::spawn(async move {
        loop {
          match rx.recv().await {
            Ok(QueueEvent::Updated) => { let _ = app_handle.emit("queue.updated", serde_json::Value::Null); }
            Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => continue,
            Err(_) => break,
          }
        }
          ...
          loop {
            ...
            for rule in rules {
              ...
              if let Some(ref lf) = rule.last_fired_at {
                if lf.starts_with(&today) { continue; }
              }
              ...
              if should_fire {
                ...
                store_a.notify(level, "Alert".to_string(), rule.message.clone(), None).await;
      ...
      let panels: &[(&str, f64, f64, f64, f64)] = &[
        ...
        // New panels
        ("costmap",      400.0, 480.0,  780.0,  610.0),
        ("decisions",    500.0, 560.0,   24.0, 1260.0),
        ...
        ("alerts",       380.0, 520.0, 1100.0,  920.0),
      ...
      let costmap_item    = MenuItem::with_id(app.handle(), "toggle-costmap",      "Cost Map",     true, None::<&str>)?;
      let decisions_item  = MenuItem::with_id(app.handle(), "toggle-decisions",    "Decisions",    true, None::<&str>)?;
      let session_item    = MenuItem::with_id(app.handle(), "toggle-session",      "Session",      true, None::<&str>)?;
      let scratchpad_item = MenuItem::with_id(app.handle(), "toggle-scratchpad",   "Scratchpad",   true, None::<&str>)?;
      let checkpts_item   = MenuItem::with_id(app.handle(), "toggle-checkpoints",  "Checkpoints",  true, None::<&str>)?;
      let logtail_item    = MenuItem::with_id(app.handle(), "toggle-logtail",      "Log Tail",     true, None::<&str>)?;
      let toollog_item    = MenuItem::with_id(app.handle(), "toggle-toollog",      "Tool Log",     true, None::<&str>)?;
      let alerts_item     = MenuItem::with_id(app.handle(), "toggle-alerts",       "Alerts",       true, None::<&str>)?;
      let queue_item      = MenuItem::with_id(app.handle(), "toggle-queue",        "Work Queue",   true, None::<&str>)?;
      ...
      let tray_menu = Menu::with_items(app.handle(), &[
        ...
        &sep2,
        &costmap_item, &decisions_item, &session_item,
        &scratchpad_item, &checkpts_item, &logtail_item, &toollog_item,
        &alerts_item, &queue_item,
        &sep3, &demo_item, &sep4,
        &quit_item,
      ])?;
      ...
      TrayIconBuilder::with_id("main")
        ...
        .on_menu_event(|app, event| match event.id().as_ref() {
          ...
          "toggle-costmap"      => toggle_panel(app, "costmap"),
          "toggle-decisions"    => toggle_panel(app, "decisions"),
          ...
          "toggle-alerts"       => toggle_panel(app, "alerts"),
          ...
          "demo-seed" => {
            ...
            tauri::async_runtime::spawn(demo_seed_inner(store, timer, tokens, scratchpad, checkpoints, logtail, queue));
          }
          "quit" => app.exit(0),
          _ => {}
        })
