...
fn row_to_item(row: &sqlx::sqlite::SqliteRow) -> KlaxonItem {
  ...
  let answered_at = answered_at_str.as_deref().and_then(|s| {
    chrono::DateTime::parse_from_rfc3339(s).ok().map(|d| d.with_timezone(&Utc))
  });

  KlaxonItem { id, level, title, message, created_at, ttl_ms: ttl_ms.map(|v| v as u64), status, form, actions, response, answered_at }
}
