    ...
    <DraggablePanel id="checkpoints" title="Checkpoints" width={420}>
      ...
      {latestPct != null && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 3, opacity: 0.7 }}>
            <span>Overall progress</span>
            <span>{latestPct}%</span>
          </div>
          <div style={{ height: 6, background: "var(--border)", borderRadius: 3, overflow: "hidden" }}>
            <div style={{
              height: "100%", width: `${latestPct}%`,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 0, overflowY: "auto", maxHeight: 360, position: "relative" }}>
        {filtered.length === 0 ? (
            ...
            <div key={cp.id} style={{ display: "flex", gap: 10 }}>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ flex: 1, fontSize: 12, fontWeight: isLatest ? 700 : 500 }}>{cp.label}</span>
      ...
      <button
        ...
        style={{
          marginTop: 10, fontSize: 11, padding: "3px 8px", borderRadius: 6,
          cursor: "pointer", background: "var(--card)", border: "1px solid var(--border)",
          color: "var(--text)", opacity: filtered.length === 0 ? 0.4 : 0.7,
        }}
      >
