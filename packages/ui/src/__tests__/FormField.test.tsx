...
describe("validateField", () => {
  ...
  it("returns Required for empty required text field", () => {
    expect(validateField({ type: "text", id: "x", label: "X", required: true }, "")).toBe("Required");
    expect(validateField({ type: "text", id: "x", label: "X", required: true }, undefined)).toBe("Required");
  });

  it("returns null when required field has value", () => {
    expect(validateField({ type: "text", id: "x", label: "X", required: true }, "hello")).toBeNull();
  });
...
describe("FormFieldRenderer", () => {
  it("renders text input", () => {
    render(<FormFieldRenderer field={{ type: "text", id: "t", label: "Name" }} value="" onChange={vi.fn()} />);
  ...
  it("calls onChange for text input", () => {
    ...
    render(<FormFieldRenderer field={{ type: "text", id: "t", label: "Name" }} value="" onChange={onChange} />);
    fireEvent.change(screen.getByRole("textbox"), { target: { value: "Alice" } });
    expect(onChange).toHaveBeenCalledWith("Alice");
  });

  it("renders textarea", () => {
    render(<FormFieldRenderer field={{ type: "textarea", id: "ta", label: "Notes" }} value="" onChange={vi.fn()} />);
    expect(screen.getByRole("textbox")).toBeInTheDocument();
  });

  it("renders select with options", () => {
    const field = { type: "select" as const, id: "s", label: "Color", options: [{ value: "red", label: "Red" }] };
    render(<FormFieldRenderer field={field} value="" onChange={vi.fn()} />);
    expect(screen.getByText("Red")).toBeInTheDocument();
  });

  it("renders radio options", () => {
    const field = { type: "radio" as const, id: "r", label: "Pick", options: [{ value: "a", label: "A" }, { value: "b", label: "B" }] };
    render(<FormFieldRenderer field={field} value="" onChange={vi.fn()} />);
    expect(screen.getAllByRole("radio")).toHaveLength(2);
  });

  it("renders checkbox", () => {
    render(<FormFieldRenderer field={{ type: "checkbox", id: "c", label: "Agree" }} value={false} onChange={vi.fn()} />);
    expect(screen.getByRole("checkbox")).toBeInTheDocument();
  });

  it("renders required asterisk", () => {
    render(<FormFieldRenderer field={{ type: "text", id: "t", label: "Name", required: true }} value="" onChange={vi.fn()} />);
    expect(screen.getByText("*")).toBeInTheDocument();
  });

  it("renders error message", () => {
    render(<FormFieldRenderer field={{ type: "text", id: "t", label: "Name" }} value="" error="Required" onChange={vi.fn()} />);
    expect(screen.getByText("Required")).toBeInTheDocument();
  });

  it("renders rating stars", () => {
    const onChange = vi.fn();
    render(<FormFieldRenderer field={{ type: "rating", id: "r", label: "Score", min: 1, max: 5 }} value={3} onChange={onChange} />);
  ...
  it("renders slider with live readout", () => {
    const onChange = vi.fn();
    render(<FormFieldRenderer field={{ type: "slider", id: "s", label: "Volume", min: 0, max: 100, step: 1 }} value={42} onChange={onChange} />);
  ...
  it("renders unsupported type fallback", () => {
    render(<FormFieldRenderer field={{ type: "datetime", id: "d", label: "When" }} value="" onChange={vi.fn()} />);
